...
{
    e->image_Enigme[0] = IMG_Load("enigme.jpg");
    e->image_Enigme[1] = IMG_Load("11111.jpg");
    e->image_Enigme[2] = IMG_Load("22222.jpg");
    e->image_Enigme[3] = IMG_Load("33333.jpg");
    e->current_frame = 0;
    ...
    e->position_correct.x = 815;
    e->position_correct.y = 380;
    e->position_wrong.x = 815;
    e->position_wrong.y = 380;

    e->police=TTF_OpenFont("police.ttf",50 );
    e->reponse_1 = NULL;
    e->pos_reponse_1.x=200;
    e->pos_reponse_1.y=750;

    e->reponse_2 = NULL;
    e->pos_reponse_2.x=800;
    e->pos_reponse_2.y=750;

    e->reponse_3 =NULL;
    e->pos_reponse_3.x=1500;
    e->pos_reponse_3.y=750;
...

{
    ...
    do {
        e->alea = 1;
    }while ( e->alea <1 || e->alea > 1 );

    fichier_reponse = fopen("rp.txt","r"); 
    fichier = fopen("q.txt","r"); 
    fichier_bonne_reponse = fopen("br.txt","r");  
...
int resolution(Enigme *e, SDL_Surface* screen)
{
    ...
    while(continuer == 1)
    {
        ...
        case SDL_MOUSEBUTTONDOWN:
        {
            ...
            {
                if ( event.button.x>200 && event.button.x<350 && event.button.y>750 && event.button.y<850) 
                {
                    if ( e->alea == 1 )
                    {
                        e->resultat= 1;
                ...
                if ( event.button.x>800 && event.button.x<950 && event.button.y>750 && event.button.y<850) 
                {
                    if ( e->alea == 1 )
                    {
                        e->resultat= 2;
                ...
                if ( event.button.x>1500 && event.button.x<1650 && event.button.y>750 && event.button.y<850) 
                {
                    if ( e->alea == 1 )
                    {
                        e->resultat= 2;
